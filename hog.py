...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def compare(score0,score1):
        if who==0:
            record=score0-last_score
            if record>running_high:
                if record>1:
                    print(f"{record}points!That's the biggest gain yet for Player0")
                else:
                    print(f"{record}point!That's the biggest gain yet for Player0")
                    return announce_highest(who, score0, max(score0 - last_score, running_high))
        else:
            record = score0 - last_score
            if record > running_high:
                if record > 1:
                    print(f"{record}points!That's the biggest gain yet for Player1")
                else:
                    print(f"{record}point!That's the biggest gain yet for Player1")
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    if opponent_score > score and hog_pile(score + 1, opponent_score):
        return 10
        # 领先时，如果获得1分会被交换，返回0
    if score > opponent_score and hog_pile(score + 1, opponent_score):
        return 0
        # 判断free bacon是否可以获胜
    max_roll = max_scoring_num_rolls(num_samples=100)
    num = hog_pile_strategy(score, opponent_score, 100 - score, max_roll)
    if num == 0:
        return 0
    # 不投0颗，是否有获胜可能
    ...
    return hog_pile_strategy(score, opponent_score, margin, max_roll)
    # Remove this line once implemented.
    # END PROBLEM 12
